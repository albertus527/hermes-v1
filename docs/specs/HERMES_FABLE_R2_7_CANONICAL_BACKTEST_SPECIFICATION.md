# HERMES FABLE R2.7 — CANONICAL BACKTEST SPECIFICATION
**Revision:** R2.7 (R2.6 authoritative baseline with only the accepted targeted deterministic release-gate corrections P-1 through P-4 integrated in place; C-1, C-2, I-1, I-2, P-A-01 through P-A-05, FP-1 through FP-8, EB-01, TRAIN_SUBSTITUTED_ZERO, and all other R2.6 normative policy are preserved without reopening; R2.6 audit category-E findings E-1 through E-13 are explicitly not applied)  
**Status:** **R2.7 — R2.6 TARGETED RELEASE-GATE PATCH APPLIED; READY FOR INDEPENDENT PATCH VERIFICATION.** No final freeze, release approval, strategy validation, profitability validation, OOS pass, paper-trading pass, broker/provider confirmation, or live readiness is claimed.
...
**Citation convention:** members of numbered lists are cited as "§*S* item *n*" (e.g., "§21 item 18"). All section references in this document refer to R2.7 sections only.
...
| News + earnings calendar | Finnhub free tier | KNOWN available; history depth (news **and earnings calendar**) MUST CONFIRM |
...
**Current status:** `R2.7 — R2.6 TARGETED RELEASE-GATE PATCH APPLIED; READY FOR INDEPENDENT PATCH VERIFICATION`

R2.7 preserves the R2.6 authoritative baseline normatively unchanged except for the accepted targeted deterministic corrections **P-1 through P-4** from the R2.6 release-gate audit. C-1, C-2, I-1, I-2, P-A-01 through P-A-05, FP-1 through FP-8, EB-01, TRAIN_SUBSTITUTED_ZERO, fee/train-substitution design, news effect mapping policy, coverage/PIT design, walk-forward architecture, acceptance criteria, thresholds, scoring weights, sizing, fee rates, and benchmark policy outside P-2's conforming references are not reopened. **R2.6 audit category-E findings E-1 through E-13 are explicitly not applied in R2.7 and remain reserved for a later editorial pass. No final freeze is claimed.**

All previously recorded R2/R2.1/R2.2/R2.3/R2.4/R2.5/R2.6 readiness or blocker statuses are **RETIRED / SUPERSEDED** as current-document statuses by this R2.7 status.
