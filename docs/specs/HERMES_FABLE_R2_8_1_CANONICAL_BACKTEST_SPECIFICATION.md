# HERMES FABLE R2.8.1 — CANONICAL BACKTEST SPECIFICATION
**Revision:** R2.8.1 (R2.8 authoritative baseline with only the accepted targeted historical NEWS-provider clarification N-1 integrated in place; the R2.8 earnings-source clarifications A-1 through A-4 with corrections C-1 through C-3, P-1 through P-4, C-1, C-2, I-1, I-2, P-A-01 through P-A-05, FP-1 through FP-8, EB-01, TRAIN_SUBSTITUTED_ZERO, and all other R2.7/R2.8 normative policy are preserved without reopening; R2.6 audit category-E findings E-1 through E-13 remain explicitly not applied)  
**Status:** **R2.8.1 — R2.8 TARGETED HISTORICAL NEWS-PROVIDER CLARIFICATION APPLIED; READY FOR INDEPENDENT PATCH VERIFICATION.** No final freeze, release approval, strategy validation, profitability validation, OOS pass, paper-trading pass, broker/provider confirmation, or live readiness is claimed.
...
**Citation convention:** members of numbered lists are cited as "§*S* item *n*" (e.g., "§21 item 18"). All section references in this document refer to R2.8.1 sections only.
...
| News headlines | **Baseline provider: Finnhub free tier (§3.8, N-1).** Finnhub is the **baseline** NEWS provider, not an exclusive mandatory provider: historical/backtest NEWS MAY use an alternative provider iff it satisfies the §3.8 historical news-source data contract, and historical/backtest NEWS and live/recent NEWS MAY originate from different providers. | Baseline KNOWN available; baseline news history depth MUST CONFIRM; historical/backtest substitute sources per §3.8 |
...
### §3.8 Historical news-source data contract (NORMATIVE — N-1)

This section is the provider-neutral contract that establishes historical NEWS provider substitutability. **N-1 (historical NEWS provider substitutability):** Finnhub free tier (§3.1) is the **baseline** NEWS provider, not an exclusive mandatory provider. Historical/backtest NEWS MAY use a different provider from the baseline/live NEWS provider, ONLY when the substitute satisfies this §3.8 contract. A substitute historical NEWS provider is permitted **only if its data can be normalized into the SAME existing canonical NEWS contract** — no new canonical NEWS row shape is introduced.

**N-1-a — Minimum historical news-source data contract.** A compliant historical news source must provide sufficient semantic information for normalization into the existing canonical NEWS fields (§11.1, §16), and no more:

- `ticker`;
- `published_at` (a timestamp of the required precision; the existing §11.1 ingestion-validity rules apply unchanged — a source value that cannot yield a valid timed `published_at` is dropped as `HEADLINE_UNTIMED`, exactly as for the baseline provider);
- `source` (publisher/source, under the existing §11.1/§16 semantics — see N-1-e);
- the raw headline text from which the existing FP-4 `headline_text_normalized` and `headline_hash` are computed; and
- fetched-at/provenance information sufficient for the existing §16 `news_headlines` row.

The normalized canonical input remains exactly the existing `news_headlines` row shape (§16). No provider-specific canonical headline field is added.

**N-1-b — Downstream invariance (NORMATIVE).** Provider substitution MUST NOT change: FP-4 normalization; `headline_hash` semantics; the ticker relationship; P-4 consistency; the canonical news classification taxonomy; classifier inputs; classification-cache identity/behavior; coverage semantics; G9; catalyst/news scoring; veto behavior; exits; or strategy behavior. For identical normalized canonical inputs and identical pinned classification inputs/cache, downstream deterministic behavior MUST be **provider-independent**. G7, the §11.2 ordered effect mapping, the §11.3 keyword fallback and two-source confirmation rule, and all §11.6 coverage semantics are consumed from the normalized canonical inputs exactly as before.

**N-1-c — Historical/live provider divergence.** Historical NEWS and live/recent NEWS MAY originate from different providers. This divergence alone is **NOT** a backtest/live semantic divergence, provided both sources satisfy the same canonical NEWS semantic contract (this §3.8 for the historical source; the existing §11.1/§11.6 semantics for the baseline provider). Any such divergence is disclosed at report level (§13.9 item 22) as descriptive report-level metadata only.

**N-1-d — Provider-generated fields are non-canonical.** Provider-specific fields that are not part of the canonical NEWS contract — in particular provider sentiment labels, provider sentiment scores, provider relevance scores, provider topic classifications, and provider-generated trading opinions — MUST NOT silently alter strategy semantics and MUST NOT substitute for the canonical pinned-model Phase-2 news classification (§11.5, P-4) unless a future canonical revision explicitly says so. They may be ignored/dropped at the adapter boundary. The only canonical classification is the existing pinned-model Phase-2 classification/cache process.

**N-1-e — Provider identity and persistence.** No new per-row provider identity field is required. The existing `source` semantics remain unchanged and are **not reinterpreted** as a provider-ID field where they currently mean publisher/source. Provider/API identity may remain report/run/provenance metadata where the existing architecture permits it. No new dataset-hash field, provider-ID field, or new `manifest_version` semantics is introduced; existing §16 schemas, existing `news_headlines`/`news_classifications` shapes, the existing cache identity, and the existing reproducibility/provenance contract remain unchanged. Existing NEWS coverage rules remain authoritative: a substitute provider does **NOT** create verified NEWS coverage merely by being queried — verified coverage must continue to satisfy the existing §11.6/§16 `coverage_manifests` semantics; missing/unverified coverage MUST NOT be converted into verified zero; and successful verified-zero spans retain the existing semantics.

**N-1-f — Ingestion-layer handling of provider API limits (NORMATIVE).** Provider-specific API limitations — including pagination, response-size limits, deterministic date/window chunking, rate limits, and incomplete responses — MUST be handled by the ingestion layer without weakening coverage semantics. An ingestion process MUST NOT attest a span as verified if provider limits make completeness for that span unestablished. A truncated, paginated, rate-limited, or otherwise incomplete provider response never silently creates verified coverage; unverified spans stay unverified exactly as §11.6 requires. This specification does not prescribe any provider-specific chunk size or window; such choices are implementation details that must preserve the completeness-attestation rule above.

This contract is deliberately provider-neutral. No specific provider (for example Alpha Vantage, a candidate historical NEWS implementation choice) is canonically required or exclusively permitted by it, and no external provider facts, pricing, quotas, response-field details, or undocumented API behavior are normatively asserted anywhere in this specification.
...
- **Backtest cache population:** the Phase-2 job classifies **every timed headline in the run-pinned, manifest-recorded covered NEWS span (§11.6; baseline or substitute provider per §3.1/§3.8), irrespective of wall-clock age**. No age-based exclusion exists at classification time.
...
22. **Backtest/live NEWS-provider divergence (N-1):** the historical/backtest NEWS provider MAY differ from the baseline/live NEWS provider (§3.8). This divergence alone is not a backtest/live semantic divergence, provided both sources satisfy the same canonical NEWS semantic contract (§3.8, §11.1, §11.6). If the historical/backtest and baseline/live NEWS providers differ, that divergence is disclosed here at **report level**. This disclosure is descriptive report-level metadata only and introduces no persistence-schema requirement (§3.8 N-1-e).
...
news_headlines(headline_hash, source, ticker, published_at,
     ...
     -- N-1 (R2.8.1): historical NEWS provider substitution (§3.8) does NOT
     -- alter this schema; substitute-provider rows use the identical shape and
     -- `source` retains its existing publisher/source semantics.
...
| Finnhub news & earnings history depth | MUST CONFIRM | Coverage rules §11.6 / §7.2 G6. N-1 (R2.8.1): Finnhub remains the **baseline** NEWS provider; historical/backtest NEWS MAY use a substitute provider satisfying the §3.8 contract, so the baseline's history depth no longer solely bounds backtest NEWS availability |
| **Historical/backtest NEWS provider substitutability (N-1)** | DECIDED | §3.8 provider-neutral contract; substitute must normalize into the same canonical NEWS row shape; FP-4/P-4/coverage semantics unchanged; provider sentiment etc. never substitute for the pinned-model Phase-2 classification; provider API limits never silently create verified coverage |
...
**Current status:** `R2.8.1 — R2.8 TARGETED HISTORICAL NEWS-PROVIDER CLARIFICATION APPLIED; READY FOR INDEPENDENT PATCH VERIFICATION`

R2.8.1 preserves the R2.8 authoritative baseline normatively unchanged except for the accepted targeted historical NEWS-provider clarification **N-1** (§3.1 news-provider row clarification, new §3.8 historical news-source data contract, §11.1 cache-population wording, §13.9 item 22 divergence disclosure, §16 `news_headlines` non-modification comment, §22 epistemic rows). R2.8 itself preserved the R2.7 authoritative baseline normatively unchanged except for the accepted targeted earnings-source clarifications **A-1 through A-4, with corrections C-1 through C-3** (§3.1 earnings-provider row split, new §3.7 earnings-source data contract, §13.9 item 21 divergence disclosure). The universe, regime logic, G1–G5, existing G6 `d(e)` mapping and timing branches and blackout window, G7–G10, scoring, news classification and News Score, sizing, stop/target/exit logic, fee logic and regulatory fees, portfolio accounting, walk-forward structure, TRAIN/TEST definitions, PASS/FAIL/NOT EVALUABLE criteria, benchmarks, drawdown rules, market-data / corporate-actions / VIX / LLM-classifier provider policy, search space, `coverage_end`, and execution assumptions are not reopened. P-1 through P-4, C-1, C-2, I-1, I-2, P-A-01 through P-A-05, FP-1 through FP-8, EB-01, TRAIN_SUBSTITUTED_ZERO, fee/train-substitution design, news effect mapping policy, coverage/PIT design, walk-forward architecture, acceptance criteria, thresholds, scoring weights, sizing, fee rates, and benchmark policy are preserved without reopening. **R2.6 audit category-E findings E-1 through E-13 are explicitly not applied in R2.8.1 and remain reserved for a later editorial pass. No final freeze is claimed.**

All previously recorded R2/R2.1/R2.2/R2.3/R2.4/R2.5/R2.6/R2.7/R2.8 readiness or blocker statuses are **RETIRED / SUPERSEDED** as current-document statuses by this R2.8.1 status.
...
## R2.8.1 PATCH APPLICATION RECORD

| Clarification | Source | Affected R2.8.1 sections | Concise clarification applied |
|---|---|---|---|
| **N-1** | Accepted historical NEWS-provider clarification | §3.1; §3.8; §11.1; §13.9 item 22; §16 (`news_headlines` comment only); §22; §23 | Finnhub free tier remains the **baseline** NEWS provider where applicable. Historical/backtest NEWS MAY use a different provider, and historical/backtest NEWS and live/recent NEWS MAY originate from different providers. A substitute historical NEWS provider is permitted ONLY if its data normalizes into the SAME existing canonical NEWS contract (§3.8 N-1-a: `ticker`, `published_at`, `source`, raw headline text for FP-4 `headline_text_normalized`/`headline_hash`, fetched-at/provenance — no new canonical NEWS row shape). Provider substitution MUST NOT change FP-4 normalization, `headline_hash` semantics, ticker relationship, P-4 consistency, classification taxonomy, classifier inputs, cache identity/behavior, coverage semantics, G9, catalyst/news scoring, veto behavior, exits, or strategy behavior; downstream deterministic behavior is provider-independent for identical normalized inputs and identical pinned classification inputs/cache (§3.8 N-1-b). The historical/live provider divergence alone is not a backtest/live semantic divergence when both sources satisfy the same canonical NEWS semantic contract; it is disclosed at report level (§3.8 N-1-c, §13.9 item 22). Provider sentiment labels/scores, relevance scores, topic classifications, and provider-generated trading opinions MUST NOT substitute for the canonical pinned-model Phase-2 classification and may be dropped at the adapter boundary (§3.8 N-1-d). No per-row provider identity field, dataset-hash field, provider-ID field, or new `manifest_version` semantics; `source` keeps its existing publisher/source semantics (§3.8 N-1-e). Existing §11.6/§16 NEWS coverage semantics remain authoritative — querying a substitute provider never creates verified coverage, and provider API limits (pagination, response-size limits, chunking, rate limits, incomplete responses) MUST be handled by the ingestion layer without attesting unestablished spans as verified; no provider-specific chunk size is prescribed (§3.8 N-1-f). The rule is provider-neutral and does not canonically require or exclusively permit any specific provider. |

R2.8.1 does not modify the existing NEWS schema (§16), FP-4, P-4, the classification taxonomy or classifier semantics, §11.6 coverage semantics, G7/G9 or any news scoring/veto behavior, the R2.8 EARNINGS clarification (§3.7) or G6, or any strategy policy (universe, regime, scoring, sizing, exits, fees, walk-forward, criteria, benchmarks, search space, `coverage_end`, execution assumptions). The §16 `news_headlines` change is a non-modification comment only. No section-number/reference repair was required by the insertion of §3.8.

R2.8.1 — R2.8 TARGETED HISTORICAL NEWS-PROVIDER CLARIFICATION APPLIED;
READY FOR INDEPENDENT PATCH VERIFICATION
